...
<x-layouts.yandex title="Заказ | {{ $order->id }}">
    ...
    <br>
    <br>
    <h5>Изменить персональные данные</h5>
    <form id="test-form" action="{{ route('orders.update', [ $order->id ]) }}" method="post" name="test-form">
        @csrf
        @method('PUT')
        ...
        <div class="form-group">
            ...
            <input id="phone" class="form-control @error('phone') is-invalid @enderror" type="text" name="phone" value="{{ $order->phone }}">
            @error('phone')<div class="text-danger">{{ $message }}</div>@enderror
        </div>

        <div class="form-group">
            <label for="">Email:</label>
            <input type="email" class="form-control @error('email') is-invalid @enderror" name="email" value="{{ $order->email }}">
            @error('email')<div class="text-danger">{{ $message }}</div>@enderror
        </div>

        <div class="form-group">
            <label for="">Введите адрес и нажмите проверить:</label>
            <input id="suggest" type="text" class="form-control @error('address') is-invalid @enderror" name="address" value="{{ $order->address }}">
